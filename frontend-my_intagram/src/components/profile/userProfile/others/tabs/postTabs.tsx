...
import { collection, doc, getDocs, setDoc, addDoc, getFirestore, getDoc, deleteDoc } from 'firebase/firestore';
...
const PostTabs: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (user) {
        const userId = user.uid;
        const postsRef = collection(postCollection, userId, 'posts');
  ...
  const handleLikeToggle = async (postId: string) => {
    ...
    const updatedPosts = posts.map((post) => {
      if (post.id === postId) {
        ...
        const likedRef = doc(postCollection, userId, "posts", postId);
        setDoc(likedRef, {
          liked: {
            [userId!]: newLiked
          }
  ...
  const handleDeletePost = async (postId: string) => {
    try{
      const userId = user?.uid
      const deleteRef = doc(postCollection, userId, "posts", postId);
          ...
          <div className="flex justify-center items-center overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
            <div className="relative w-[80%] md:w-auto mx-auto">
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                {selectedPost && (
                  <div>
                    <div className='px-3 pt-3 rounded-full flex justify-between' onClick={() => setModal(false)}>
                      <button className='ml-2' onClick={() => handleDeletePost(selectedPost.id)}>
