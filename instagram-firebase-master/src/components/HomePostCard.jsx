...
const HomePostCard = ({ post }) => {                     //headerheaderheaderheaderheaderheaderheaderheaderheaderheader
  ...
  return (
    <>
        ...
        <div className="nickname flex-grow">
          <Link to={`/${post?.user?.username}`} className="font-semibold">
            {post?.user?.username}
          </Link>
          <p>•Behruz Xoldorov • Original audio</p>
        </div>
        <button>
          <PostMenuIcon />
        </button>
    ...
    >
      ...
      <div className="comment sm:block sm:border-t-[1px] text-slate-900 p-3 border-slate-500/30">
        <form onSubmit={commentSubmit}>
          <div className="flex items-center gap-3">
            ...
            <div className="button5">
              <button className="button-6" onClick={deletePost}>
                Delete 
              </button>
            </div>
          </div>
