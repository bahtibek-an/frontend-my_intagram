...
const Publications = ({ setOpen, currentUser, users, getUsers, userName, id, dark }) => {
    ...
    const deletePost = async() => {
        const filteredPublications = currentUser.publications.filter((post)=>{
            return post.id !== publid
        })
        await setDoc(doc(db, "users", `${id}`), {...currentUser, publications : [...filteredPublications]})
    ...
    return (
        <>
            ...
            {shareWindow &&
                ...
                {deleteWin && <div className="delete_div"
                onClick={()=>setDeleteWin(false)}>
                    <div className="delete"
                    style={{backgroundColor: dark ? "#3f3f3f" : "#efefef"}}>
                        <button
