...
const Post = ({ post }) => {
  ...
  const handleLikeClick = async (e) => {
    ...
    if (currentUser) {
      ...
      try {
        const isPostLiked = localStorage.getItem(`liked_${id}`);
        const isPostDisliked = localStorage.getItem(`disliked_${id}`);
        ...
        if (!isPostLiked && !isPostDisliked) {
          ...
          // Mark the post as liked in localStorage
          ...
          // Mark the post as unliked in localStorage
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      // Check localStorage for the like status
      const isPostLiked = localStorage.getItem(`liked_${id}`);
      ...
      <div className="card shadow w3-card">
        <div className="card-header text-capitalize d-flex justify-content-between">
          ...
          {(currentUser.uid === post.userId && (
            <div class="dropdown">
              <button
                class="btn d-flex fs-5 z-index-2"
                type="button"
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                <li>
                  <a class="dropdown-item" onClick={handleDeleteClick}>
                    Delete Post
                  </a>
                </li>
              </ul>
        ...
        <div className="card-body">
          ...
          <div
            ...
            className={`likes text-dark fs-5 ${isLiked ? "liked" : ""}`}
          >
            {isLiked ? (
              <span>❤️ {likeCount}</span>
            ) : (
          ...
          <p className="card-text">
            {post.comments > "" &&
              post.comments
                .slice(Math.max(post.comments.length - 2, 0))
                .map((c, index) => (
                  <div className="d-flex align-items-center">
                    <p key={index}>
                      <ion-icon name="send"></ion-icon> {c}
                    </p>
                  </div>
                ))}
          </p>
          <div className="input-group mb-3">
            <span
              className="input-group-text btn-insta"
