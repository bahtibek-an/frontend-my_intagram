...
import { Firestoredb, db, storage } from '../firebase';
    ...
    const handleSubmit = async (event) => {
        ...
        if (imageFile && storage) {
            try {
                ...
                const postHours = new Date().getHours();
                const postMinuts = new Date().getMinutes();
                ...
                if (snapshot && snapshot.ref) {
                    ...
                    await addDoc(postsCollection, {
                        ...
                        likes: [],
                        date: `${postHours}:${postMinuts} ${postDate}`,
                    });
            ...
            console.error("Storage or imageFile is undefined.");
            setLoading(false);
        }
    };
    ...
    return (
        <>
            <nav>
                ...
                <ul className="list">
                    <li><a href="/">Home</a></li>
                    <li><a onClick={openModal}>Add Post</a></li>
                    <li><a href={`/${currentUser.uid}`}>Profile</a></li>
                </ul>
                <button className="search" onClick={openSearch}></button>
            </nav>
                                ...
                                <div className="">
                                    <label className="">
                                        <input
                                            ...
                                            required
                                        />
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className='upload-image'>
            ...
            {searchVisible && (
                <div id="myModal" className="modal" onClick={handleClickOutside}>
                    <div className="modal-content">
                        ...
                        <h2>Instagram - Search</h2>

                        <input type="text" id="myInput" value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            placeholder="Search by username" title="Type username" />
                            ...
                            {searchResults.length === 0 ? (
                                ...
                                <li className="space-y-2">
                                    {searchResults.map((user) => (
                                        <Link key={user.id} to={`/${user.uid}`} onClick={closeSearch}>
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" className='user-icon'>
